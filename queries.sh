...
echo -e "\nList of teams who played in the 2014 'Eighth-Final' round:" #FALTA
echo "$($PSQL "SELECT name FROM teams join games on teams.team_id=games.opponent_id where round='Eighth-Final' and year=2014  UNION  SELECT name FROM teams join games on teams.team_id=games.winner_id where round='Eighth-Final' and year=2014 order by name  ")"
